...
router.post("/", async (req, res) => {
  ...
  if (!name || name === "") {
    req.flash("error", "The person name is required.");
    res.redirect(req.baseUrl + "/new", 302);
  } else {
    db.execute(`INSERT INTO person (name, alive) VALUES ('${name}', ${1})`)
      .then(() => {
        req.flash("success", `${name} was created.`);
        res.redirect(req.baseUrl, 302);
      })
      .catch((err) => {
        req.flash("error", `${name} could not be create due to error: ${err?.message}`);
        res.redirect(req.baseUrl + "/new", 302);
      });
  }
});
