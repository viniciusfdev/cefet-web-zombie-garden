...
router.get("/", async (req, res, next) => {
  try {
    ...
    res.format({
      default: () => res.render("list-zombies", { zombies }),
      "application/json": () => res.json({ zombies }),
    });
  } catch (error) {
    console.error(error);
...
router.post("/brains", async (req, res, next) => {
  ...
  try {
    ...
    if (!insertResult || insertResult.insertedRows < 1) {
      throw new Error(
        `Não inseriu um novo zumbi na tabela graças à pessoa com id ${personId}`
      );
    }
...
function getPictureIndex() {
  ...
  return (
    Math.floor(Math.random() * (ZOMBIE_PICTURES_LENGTH - ZOMBIE_FIRST_PICTURE_INDEX)) +
